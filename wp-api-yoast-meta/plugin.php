...
class Yoast_To_REST_API {
	...
	function wp_api_encode_yoast( $p, $field_name, $request ) {
		...
		$yoast_meta = array(
			'yoast_wpseo_title'     => html_entity_decode($wpseo_frontend->get_content_title()),
			'yoast_wpseo_metadesc'  => html_entity_decode($wpseo_frontend->metadesc( false )),
			'yoast_wpseo_canonical' => html_entity_decode($wpseo_frontend->canonical( false ))
		);
	...
	private function wp_api_encode_taxonomy() {
		...
		$yoast_meta = array(
			'yoast_wpseo_title'    => html_entity_decode($wpseo_frontend->get_taxonomy_title()),
			'yoast_wpseo_metadesc' => html_entity_decode($wpseo_frontend->metadesc( false )),
		);
